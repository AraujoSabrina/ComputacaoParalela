...
import threading
import time
from distributed_messaging_system import DistributedMessagingSystem
...
def monitor_messages(client: Client, duration: int):
...
def resource_usage_test(system: DistributedMessagingSystem):
    print("\n===== TESTE DE EXCLUSÃO MÚTUA =====")
    
    system.create_resource("impressora")
    
    def use_resource(client: Client, resource_id: str, delay: float = 0):
        time.sleep(delay)
...
def messaging_test(system: DistributedMessagingSystem):
    print("\n===== TESTE DE MENSAGENS =====")
...
if __name__ == "__main__":
    ...
    system = DistributedMessagingSystem()
